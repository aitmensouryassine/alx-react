...
describe('Test notificationReducer', () => {
  ...
  it('Test that MARK_AS_READ returns the data with the right item updated', () => {
    ...
    const expected_state = initial_state.setIn(['notifications', 'entities', 'messages', action.index, 'isRead'], true);
  ...
  it('Test that SET_LOADING_STATE changes loading attribute', () => {
    const initial_state = fromJS({
      filter: NotificationTypeFilters.DEFAULT,
      notifications: notificationsNormalizer([
        { id: 1, type: 'default', value: 'New course available', isRead: false },
    ...
    const received_state = notificationReducer(initial_state, action);
    const expected_state = initial_state.set('loading', action.loadingState);
